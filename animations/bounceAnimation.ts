...
export const bounceEnterAnimation =
  (durationMs: number = 350, delayMs: number = 0, animationFunction: string = 'cubic-bezier(0, 0, 0, 1.4)') =>
  (element: HTMLElement) => {
...
export const bounceLeaveAnimation =
  (durationMs: number = 400, delayMs: number = 0) =>
  (element: HTMLElement) => {
    return new Promise(async (resolve, reject) => {
      // Initial styling
      element.style.position = 'absolute';
